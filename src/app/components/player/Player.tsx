...
class Player extends Component<PlayerPropType, PlayerStateType> {
  static getDerivedStateFromProps(props: any) {
    const newId = props.playlist[props.currentIndex] && props.playlist[props.currentIndex]
    if (newId !== currentId) {
      return {
        songReady: false,
  ...
  playHandler = () => {
    if (!this.state.songReady || !this.audio.current) return;
  ...
  middleTouchMove= (e: TouchEvent<Element>) => {
    if (!this.touch.initiated || !this.lyricList.current || !this.middleL.current || !this.lyricList.current.wrapper.current) return;
  ...
  middleTouchEnd = () => {
    if (!this.lyricList.current || !this.middleL.current || !this.lyricList.current.wrapper.current) return;
    let offsetWidth;
    let opacity;
  ...
  selectItem = (item: ISong, index: number) => {
    if (this.props.mode === playMode.random) {
      index = this.findIndex(this.props.playlist, item)
    }
    this.props.setCurrentIndex(index);
    this.props.setPlaying(true);
  }
  ...
  scrollToCurrent = (scroller: any, scrollSon: NodeList, current: ISong = this.state.currentSong) => {
    const index = this.findIndex(this.props.sequenceList, current);
      ...
      <div className="player" style={{display: `${styleDisplay}`}}>
        <div className="normal-player" style={{display: `${fullScreen? 'block': 'none'} `}}>
          <div className="background">
            <img src={currentSong && currentSong.image} width="100%" height="100%"alt=""/>
          </div>
          <div className="top">
              ...
              <div className="middle-l" ref={this.middleL}>
                <div className="cd-wrapper">
                  <div className={this.cdCls() + ' cd'}>
                    <img className="image" src={currentSong && currentSong.image} alt=""/>
                  </div>
                </div>
                    ...
                    <div ref={this.lyricLine}>
                      {
                        currentLyric.lines.map((line: {txt: string}, index: number) => (
                          <p className={"text" + (currentLineNum === index? ' current' : '')} key={index}>
                            {line.txt}
                          </p>
                        ))
                      }
        ...
        <div className="mini-player" style={{display: `${!fullScreen? 'flex' : 'none'}`}} onClick={this.open}>
          <div className="icon">
            <img src={currentSong && currentSong.image} width="40" height="40" className={this.cdCls()}/>
          </div>
          <div className="text">
          ...
          <div className="control" style={{position: 'relative'}}>
            <ProgressCircle percent={this.percent()} radius={radius}>
              <i className={this.miniIncon() + ' icon-mini'} onClick={this.toggleMiniPlaying}></i>
            </ProgressCircle>
          </div>
        ...
        <PlayList
          ...
          scrollToCurrent={this.scrollToCurrent}></PlayList>
          <audio
            ref={this.audio}
