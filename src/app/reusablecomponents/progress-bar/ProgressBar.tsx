...
interface ProgressBarPrototype {
  percent: number;
  onProgressBarChange: Function;
}

interface ProgressBarStatePrototype {

}

export default class ProgressBar extends Component<ProgressBarPrototype, ProgressBarStatePrototype> {
  ...
  constructor(props: ProgressBarPrototype) {
    super(props);
  ...
  progressClick = (e: MouseEvent<Element>) => {
    if (!this.progressBar.current) return;
  ...
  _triggerPercent() {
    if (!this.progress.current || !this.progressBar.current) return;
    const barWidth = this.progressBar.current.clientWidth - progressBtnWidth;
    const percent = this.progress.current.clientWidth / barWidth;
    this.props.onProgressBarChange(percent);
  }
  progressTouchStart = (e: TouchEvent<Element>) => {
  ...
  progressTouchMove = (e: TouchEvent<Element>) => {
    if (!this.progressBar.current) return; 
  ...
  progressTouchEnd = (e: TouchEvent<Element>) => {
