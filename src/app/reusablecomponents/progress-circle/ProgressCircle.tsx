...
export default class ProgressCircle extends Component<ProgressCirclePropType, ProgressCircleStateType> {
  ...
  dashOffset = () => {
    if (Object.is(this.props.percent, NaN)) return;
    return (1 - this.props.percent) * this.state.dashArray;
  }

  render() {
    const { radius = 100 } = this.props;
    ...
    return (
      <div className="progress-circle">
        <svg width={radius} height={radius} viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
          ...
          <circle className="progress-bar"  r="50" cx="50" cy="50" fill="transparent" strokeDasharray={dashArray} strokeDashoffset={this.dashOffset()}></circle>
        </svg>
        {
          this.props.children
        }
